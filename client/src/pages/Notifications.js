...
function Notifications() {
  ...
  const markAllAsSeen = async () => {
    try {
      ...
      const response = await axios.post(
        '/api/user/mark-unseen-notifications-as-seen',
        { userId: user._id },
        {
          headers: {
  ...
  const deleteAll = async () => {
    try {
      dispatch(showLoading());
      const response = await axios.post(
        '/api/user/delete-seen-notifications',
        { userId: user._id },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
      ...
      <Tabs>
        <Tabs.TabPane tab='Unseen' key={0}>
          ...
          {user?.unseenNotifications.map((notification, index) => (
            <div key={index} className='card p-2 mt-2' onClick={() => navigate(notification.onClickPath)}>
